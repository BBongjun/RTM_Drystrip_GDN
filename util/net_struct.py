...
def get_fc_graph_struc_from_map(feature_map):
    """
    Generate a fully connected graph structure from a list of features.
    
    Args:
        feature_map (list): A list of feature names (columns).
    
    Returns:
        dict: A fully connected graph structure where each feature is connected to all others.
    """
    struc_map = {}
    
    for ft in feature_map:
        # Initialize adjacency list for each feature
        struc_map[ft] = [other_ft for other_ft in feature_map if other_ft != ft]
