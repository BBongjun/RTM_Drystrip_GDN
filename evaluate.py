...
def get_full_err_scores_without_sma(test_result, val_result):
    # np_test_result = np.array(test_result)
    # np_val_result = np.array(val_result)
    ...
    for i in range(feature_num):
        ...
        scores = get_err_socres_without_sma(test_re_list) # test anomaly score
        normal_dist = get_err_socres_without_sma(val_re_list) # val anomaly score = normal
...
def get_err_socres_without_sma(test_res):
    test_predict, test_gt = test_res
...
def get_val_performance_data(total_err_scores, normal_scores, gt_labels, topk=1):
    ...
    # validation의 max score를 threshold로 설정
